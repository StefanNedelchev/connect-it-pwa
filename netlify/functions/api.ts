...
const vapidKeys = {
  private: 'tU-hQrEKtJCVvf-cldX9tRFNYI63cM8pUeFscOXZ8rA',
  public: 'BJdFEw-MCEfM5xutSUOCA7DXGvYbnfJ2o6QNIeL1MnUJAt4Atn3QZB271PJHvowpeaHfLWlQJz13wxds_Q8h9UA',
};
...
router.post('/single-newsletter', (req: Request<unknown, unknown, { endpoint: string }>, res) => {
  const sub = inMemoryDb.find((s) => s.endpoint === req.body.endpoint);
...
router.post('/send-message', (req, res) => {
  const sendPromises = inMemoryDb.map((sub) => (
    send(sub).catch((err: WebPushError) => {
      if (err.statusCode === 404 || err.statusCode === 410) {
        console.log('Subscription has expired or is no longer valid: ', err.message);
        inMemoryDb = inMemoryDb.filter((s) => s.endpoint !== sub.endpoint);
      } else {
